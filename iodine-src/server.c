...
#define topdomain vars->topdomain
#define password vars->password
static struct encoder *b32;
static struct encoder *b64;
static struct encoder *b64u;
static struct encoder *b128;
#define created_users vars->created_users

#define check_ip vars->check_ip
#define my_mtu vars->my_mtu
#define my_ip vars->my_ip
#define netmask vars->netmask

#define ns_ip vars->ns_ip

#define bind_port vars->bind_port
static int debug;
...
void set_vars(struct iodine_server_vars *src) {
	vars = src;
}
...
int get_external_ip(struct in_addr *ip)
{
	int sock;
...
int
server_tunnel(int tun_fd, int dns_fd, int bind_fd, int max_idle_time)
{
	struct timeval tv;
	fd_set fds;
	int i;
